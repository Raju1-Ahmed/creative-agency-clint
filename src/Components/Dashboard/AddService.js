...
const AddService = () => {
    ...
    const onSubmit = data => {
        const url = `https://creative-agency-c2st.onrender.com/service`;
        fetch(url, {
            method: 'POST',
            headers: {
    ...
    return (
        <div className='mt-9'>
            ...
            <div className='grid grid-cols-1 justify-items-center ml-5 '>
                <form  onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="pt-2">
                    <input placeholder='Price' type="number" {...register("price")}  className="input w-96 input-bordered" />
                    </div>
                    <input className='btn btn-wide mt-2 sm:btn-sm md:btn-md lg:btn-wide   text-white' type="submit" value="Send" />
                </form>
            </div>
        </div >
    );
